...
class TrendReq(object):
    ...
    def interest_over_time(self):
        ...
        for response in trend_req_json:
            ...
            response['date'] = datetime.fromtimestamp(float(response['time'])).strftime('%Y-%m-%d')
            for kw in self.kw_list:
                response['keyword'] = kw

        # Commented code below is from original library in which we were returning a Pandas dataframe.
        # To avoid this, and instead return a json response (dict), we use the above code labled 'MODIFIED'
